...
function Vans() {
  ...
  //making use of defer and Await component for better user experience
  ...
  // }
  //removed because added errorElement here.
  ...
  //one more way
  function renderVanElememnts(vans) {
    const filteredVans = typeFilter
      ? vans.filter((van) => van.type === typeFilter)
      : vans;
    const vanElements = filteredVans.map((van) => (
  ...
  return (
    <div className="van-list-container">
      ...
      <Suspense fallback={<h2>Loading...</h2>}>
        <Await resolve={dataPromise.vans}>{renderVanElememnts}</Await>
      </Suspense>
    </div>
  );
}
...
//If the transition to new route isn't taking too much then there's no need to add defer, Await component and Suspense.